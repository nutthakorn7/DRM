...
/// <summary>
/// Applies the database schema at startup. zcrDRM intentionally has no EF migrations:
/// schema upgrades ride on EnsureCreated() (a no-op on an existing DB) plus idempotent
/// ExecuteSqlRaw patches (CREATE / ALTER ... IF NOT EXISTS), branched per provider.
/// Moved verbatim from Program.cs — the SQL text and execution order are unchanged.
/// </summary>
internal static class DatabaseInitializer
{
    public static void Initialize(AppDbContext dbContext, string auditChainKey, ILogger logger)
    {
        ...
        if (backfilledChainRows > 0)
            logger.LogInformation("Audit chain backfill: created {Count} chain rows for pre-existing events.", backfilledChainRows);
    }
}
